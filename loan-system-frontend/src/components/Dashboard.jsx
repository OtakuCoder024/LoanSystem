...
function Dashboard() {
    ...
    const fetchLoans = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/loans?userId=${user.id}&role=${user.role}`,
        );
  ...
  const handleStatus = async (loanId, status) => {
    try {
      ...
      setLoans((prev) =>
        prev.map((l) =>
          l.id === loanId ? { ...l, status: updated.status } : l,
        ),
      );
    } catch (error) {
      console.error(error);
      ...
      <div className="main">
        ...
        <div className="content">
          <div className="main-header">
            <h2>Dashboard</h2>
            {!isAdmin && (
              <button onClick={() => setShowForm(true)}>Apply for Loans</button>
            )}
          </div>
          ...
          <div className="cards">
            <div>
              ...
              <h2>{loans.filter((l) => l.status === "APPROVED").length}</h2>
            </div>
            <div>
